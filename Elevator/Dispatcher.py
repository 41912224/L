...
class Dispatcher():
    def __init__(self, ui):
        ...
        self.toUp = [[0] * 21 for i in range(3)]  # 尚未处理的上行请求
        self.toDown = [[0] * 21 for i in range(3)]  # 尚未处理的下行请求
        self.upSeq = [[] for i in range(3)]  # 电梯上行序列
        self.downSeq = [[] for i in range(3)]  # 电梯下行序列
        ...
        for i in range(1, 3):  # 加载线程
            self.thread(i)

    def listenUpBtn(self, btn_floor):  # 监听上行按钮
        self.ui.up_btn[btn_floor].setStyleSheet("QPushButton{border-image: url(D:/icon/up_pressed.png)}")
        distance = [100, 100, 100, 100, 100]  # 到达目标楼层的距离
        btn_above = {}  # 请求是否在电梯上方
        for i in range(1, 3):
            btn_above[i] = False
    ...
    def listenDownBtn(self, btn_floor):
        self.ui.down_btn[btn_floor].setStyleSheet("QPushButton{border-image: url(D:/icon/down_pressed.png)}")
        distance = [100, 100, 100, 100, 100]  # 各台电梯距离请求的距离
        btn_below = {}  # 请求是否在电梯下方
        for i in range(1, 3):  # 初始化
            btn_below[i] = False
        ...
        if self.isGoingUp[lift_num] == False and self.isGoingDown[lift_num] == False:
            if self.currentFloor[lift_num] > btn_floor:
                self.ui.floor_btn[lift_num][btn_floor].setStyleSheet(
                    "QPushButton{border-image: url(D:/icon/" + str(btn_floor) + "_pressed.png)}")
        ...
        while len(self.upSeq[lift_num]):  # 上行序列非空时
            ...
            if dist == 0:
                self.ui.floor_btn[lift_num][up_seq1].setStyleSheet(
                    "QPushButton{border-image: url(D:/icon/" + str(self.upSeq[lift_num][0]) + "_hover.png)}"
                    "QPushButton:hover{border-image: url(D:/icon/" + str(self.upSeq[lift_num][0]) + ".png)}"
                    "QPushButton:pressed{border-image: url(D:/icon/" + str(self.upSeq[lift_num][0]) + "_pressed.png)}")
            i = 1
            while i <= dist:
                if up_seq1 == self.upSeq[lift_num][0]:  # up_seq1还是移动距离最短的请求
                    if i == 1 and dist == 1:
                        time.sleep(4 * floor_time)
                    elif i == 1 or dist == i:
                        time.sleep(2 * floor_time)
                    else:
                        time.sleep(1 * floor_time)
        ...
        while len(self.downSeq[lift_num]):
            ...
            if dist == 0:
                self.ui.floor_btn[lift_num][self.downSeq[lift_num][0]].setStyleSheet(
                    "QPushButton{border-image: url(D:/icon/" + str(self.downSeq[lift_num][0]) + "_hover.png)}"
                    "QPushButton:hover{border-image: url(D:/icon/" + str(self.downSeq[lift_num][0]) + ".png)}"
                    "QPushButton:pressed{border-image: url(D:/icon/" + str(self.downSeq[lift_num][0]) + "_pressed.png)}")
            i = 1
            while i <= dist:
